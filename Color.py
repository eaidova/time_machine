...
parser = argparse.ArgumentParser(description='Run colarization with OpenVINO')
parser.add_argument('-m1', dest='model_1', default='colorization-v2', help='Path to the model_1')
parser.add_argument('-i', '--input', type=str, required=True, help='Required. Path to input image')
...
def color_image(input_image_path, model_path):
    ...
    # Setup networks
    ie = IECore()
    net_1 = ie.read_network(model_path + '.xml', model_path + '.bin')
    net_1.batch_size = 1
    ...
    cv.imwrite('test_color.png', colorize_image)
